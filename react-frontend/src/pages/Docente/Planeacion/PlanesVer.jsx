...
const PlanesVer = () => {
  ...
  const cargarPlanes = useCallback(
    (termino = "") => {
      ...
      const endpoint = termino
        ? `http://127.0.0.1:8000/api/planes/buscar?username=${username}&termino=${encodeURIComponent(
            termino
          )}`
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredPlanes.length / planesPerPage)
  );
  const startIndex = (currentPage - 1) * planesPerPage;
  const currentPlanes = filteredPlanes.slice(
    startIndex,
    startIndex + planesPerPage
  );
